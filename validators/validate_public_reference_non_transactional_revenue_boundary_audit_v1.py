#!/usr/bin/env python3
"""Validate Sprint 130 — Public Reference Non-Transactional Revenue Boundary Audit v1."""
...
AUDIT_MD = "PUBLIC_REFERENCE_NON_TRANSACTIONAL_REVENUE_BOUNDARY_AUDIT_V1.md"
SPRINT = "SPRINT_130_PUBLIC_REFERENCE_NON_TRANSACTIONAL_REVENUE_BOUNDARY_AUDIT_V1.md"
JSON_PATH = "data/public-reference-non-transactional-revenue-boundary-audit-v1.json"
SCHEMA_PATH = "data/public-reference-non-transactional-revenue-boundary-audit-v1.schema.json"
...
def validate_artifacts() -> bool:
    ok = True
    for rel in (AUDIT_MD, SPRINT, JSON_PATH, SCHEMA_PATH, SPRINT129_AUDIT):
...
def validate_audit_json() -> bool:
    ...
    expected = {
        ...
        "value_legibility_without_revenue_surface_confirmed": True,
        "public_reference_value_as_maturity_confirmed": True,
        "unsafe_revenue_boundary_language_count": 0,
    ...
    for flag in DRIFT_FLAGS:
        if data.get(flag) is not False:
            ...
            ok = False
    inventory = data.get("revenue_boundary_language_inventory", [])
    if len(inventory) < 32:
        error(f"{JSON_PATH}: must include at least 32 revenue-boundary language records")
        ok = False
    if data.get("total_revenue_boundary_language_inventory_count") != len(inventory):
        error(f"{JSON_PATH}: total_revenue_boundary_language_inventory_count mismatch")
        ok = False
    safe = sum(1 for x in inventory if x.get("status") == "safe")
    repaired = sum(1 for x in inventory if x.get("status") == "repaired")
    if data.get("safe_revenue_boundary_language_count") != safe:
        error(f"{JSON_PATH}: safe_revenue_boundary_language_count mismatch")
        ok = False
    if data.get("repaired_revenue_boundary_language_count") != repaired:
        error(f"{JSON_PATH}: repaired_revenue_boundary_language_count mismatch")
        ok = False
    for rec in inventory:
        for field in REQUIRED_LANGUAGE_FIELDS:
            if field not in rec:
                error(f"{rec.get('revenue_boundary_language_id', '?')}: missing field {field}")
                ok = False
    scenarios = data.get("walkthrough_scenarios", [])
    if len(scenarios) < 32:
        error(f"{JSON_PATH}: must include at least 32 scenarios")
        ok = False
...
def validate_no_expansion() -> bool:
    ...
    if any(f.get("public_file_id") == "PUB-FILE-0105" for f in files):
        error("PUB-FILE-0105 must not exist after Sprint 130 audit-only sprint")
        ok = False
...
def validate_revenue_hardening() -> bool:
    ok = True
    ...
    nar = (ROOT / "narrative/why-evidence-risk/index.html").read_text(encoding="utf-8")
    ...
    if 'id="non-transactional-revenue-boundary"' not in sri:
        error("strategic-review index missing non-transactional-revenue-boundary hardening")
        ok = False
    if "non-transactional-revenue-boundary" not in home:
        error("homepage missing revenue boundary cross-link")
        ok = False
    if "non-transactional-revenue-boundary" not in arh:
        error("acquisition-readiness hub missing revenue boundary cross-link")
        ok = False
    if 'id="non-transactional-revenue-boundary"' not in cit:
        error("citation-orientation missing revenue boundary hardening")
        ok = False
    if "non-transactional-revenue-boundary" not in rs:
        error("reading-sequences missing revenue boundary link")
        ok = False
    if "Commercial competitors may promise" in nar:
        error("why-evidence-risk still contains Commercial competitors phrasing")
        ok = False
    if "non-transactional-revenue-boundary" not in rpi:
        error("reviewer-packet missing revenue boundary link")
        ok = False
    if "non-transactional-revenue-boundary" not in smap:
        error("system-map missing revenue boundary link")
        ok = False
...
def validate_governance_wiring() -> bool:
    ok = True
    if not any(c.get("claim_id") == "CLAIM-0131" for c in load_json("data/evidence-ledger.json").get("claims", [])):
        error("CLAIM-0131 missing")
        ok = False
    if not any(g.get("gate_id") == "PUB-GATE-0124" for g in load_json("data/publisher-quality-gates.json").get("gates", [])):
        error("PUB-GATE-0124 missing")
        ok = False
    val_all = (ROOT / "validators/validate_all.py").read_text(encoding="utf-8")
    if "validate_public_reference_non_transactional_revenue_boundary_audit_v1.py" not in val_all:
        error("validate_all.py missing Sprint 130 validator wiring")
        ok = False
    return ok
...
def main() -> int:
    checks = [
        ...
        validate_revenue_hardening(),
        validate_terms_and_decision(),
        validate_governance_wiring(),
    ]
    if all(checks):
        print("PASS: Public Reference Non-Transactional Revenue Boundary Audit v1")
        return 0
    return 1
