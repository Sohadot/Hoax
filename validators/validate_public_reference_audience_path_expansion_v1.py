#!/usr/bin/env python3
"""Validate Sprint 113 — Public Reference Audience Path Expansion v1."""
...
INDEX_DOC = "PUBLIC_REFERENCE_AUDIENCE_PATH_EXPANSION_V1.md"
AUDIT_DOC = "PUBLIC_REFERENCE_AUDIENCE_PATH_EXPANSION_AUDIT_V1.md"
STANDARD_DOC = "PUBLIC_AUDIENCE_PATH_STANDARD_V1.md"
INDEX_JSON = "data/public-reference-audience-path-expansion-v1.json"
INDEX_SCHEMA = "data/public-reference-audience-path-expansion-v1.schema.json"
SPRINT_DOC = "SPRINT_113_PUBLIC_REFERENCE_AUDIENCE_PATH_EXPANSION_V1.md"
INDEX = "index.html"
...
ROUTE_IDS = [f"ROUTE-{i:04d}" for i in range(89, 94)]
PUB_FILE_IDS = [f"PUB-FILE-{i:04d}" for i in range(89, 94)]
...
def validate_artifacts() -> bool:
    ...
    if data.get("decision_ref") != "DEC-131":
        error("decision_ref must be DEC-131")
        ok = False
    ...
    if data.get("system_map_updated") is not True:
        ...
        ok = False
    if data.get("route_group_pages_updated") is not True:
        error("route_group_pages_updated must be true")
        ok = False
    ...
    for comp in (
        ...
        "conceptual_thesis",
        "contextual_frame",
        "audience_path_purpose",
        "audience_route_map",
        "human_use",
        "ai_retrieval_use",
        "confusion_this_path_prevents",
        "reference_answer",
        "source_confidence",
        "cite_this_reference",
        "retrieval_capsule",
...
def validate_homepage() -> bool:
    ...
    if "Audience Paths" not in content:
        error("homepage must include Audience Paths section")
        ok = False
...
def validate_system_map() -> bool:
    ...
    if "Audience Path Layer" not in content:
        error("/system-map/ must include Audience Path Layer section")
        ok = False
    for path in NEW_PATHS:
        if f'href="{path}' not in content and f"href='{path}" not in content:
...
def validate_route_group_pages() -> bool:
    ok = True
    for rel in ROUTE_GROUP_PAGES:
        content = (ROOT / rel).read_text(encoding="utf-8")
        if "/audience-paths/" not in content:
            error(f"{rel}: must link to at least one audience-path page")
            ok = False
    return ok
...
def validate_new_page(rel: str) -> bool:
    ...
    if existing_ref < 6:
        ...
        ok = False
    if 'href="/audience-paths/"' not in content and "href='/audience-paths/'" not in content:
        error(f"{rel}: must link to /audience-paths/")
        ok = False
    route_group_deepening = sum(
        1
        for p in (
    ...
    if route_group_deepening < 2:
        error(f"{rel}: must link to at least 2 route-group deepening pages")
        ok = False
    sibling_links = sum(1 for p in NEW_PATHS if f'href="{p}' in content or f"href='{p}" in content)
    if sibling_links < 2:
        error(f"{rel}: must link to at least 2 sibling audience-path pages")
        ok = False
...
def validate_governance() -> bool:
    ok = True
    if "DEC-131" not in (ROOT / "DECISION_LOG.md").read_text(encoding="utf-8"):
        error("DEC-131 missing")
        ok = False
    if "validate_public_reference_audience_path_expansion_v1.py" not in (
    ...
    for loc in SOURCE_LOCS + ["validators/validate_public_reference_audience_path_expansion_v1.py"]:
        if loc not in locs:
            error(f"source registry missing {loc}")
            ok = False
    if not any(c.get("claim_id") == "CLAIM-0114" for c in load_json("data/evidence-ledger.json").get("claims", [])):
        error("CLAIM-0114 missing")
        ok = False
    if not any(g.get("gate_id") == "PUB-GATE-0107" for g in load_json("data/publisher-quality-gates.json").get("gates", [])):
        error("PUB-GATE-0107 missing")
        ok = False
    if "Sprint 113 | COMPLETE | G113 passed" not in (ROOT / "MASTER_EXECUTION_PLAN.md").read_text(encoding="utf-8"):
        error("master execution plan missing Sprint 113 row")
        ok = False
...
def main() -> int:
    ...
    if not validate_system_map():
        ok = False
    if not validate_route_group_pages():
        ok = False
